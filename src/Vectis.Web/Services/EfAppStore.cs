...
public sealed class EfAppStore : IAppStore
{
    ...
    private static async Task<AppState> LoadUnsafeAsync(VectisDbContext db)
    {
        return new AppState
        {
            ...
            NotificationPreferences = await db.NotificationPreferences.AsNoTracking().Select(item => new NotificationPreferences(item.FamilyId, item.AutomaticEmailEnabled, item.StockLowEnabled, item.ExpiringSoonEnabled, item.PreparedBottleAgingEnabled, item.StockLowBottleThreshold, item.ExpiringSoonHours, item.PreparedBottleAgeMinutes)).ToListAsync(),
    ...
    private static async Task SaveUnsafeAsync(VectisDbContext db, AppState state)
    {
        ...
        db.NotificationPreferences.AddRange(state.NotificationPreferences.Select(item => new NotificationPreferencesEntity { FamilyId = item.FamilyId, AutomaticEmailEnabled = item.AutomaticEmailEnabled, StockLowEnabled = item.StockLowEnabled, ExpiringSoonEnabled = item.ExpiringSoonEnabled, PreparedBottleAgingEnabled = item.PreparedBottleAgingEnabled, StockLowBottleThreshold = item.StockLowBottleThreshold, ExpiringSoonHours = item.ExpiringSoonHours, PreparedBottleAgeMinutes = item.PreparedBottleAgeMinutes }));
